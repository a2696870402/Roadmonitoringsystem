        ...
        <div class="inner" style="height: 109%;">
            ...
            <div class="right_cage">
                ...
                <div class="dataAllBorder01 cage_cl" style="margin-top: 9% !important; height: 30%">
                    <div class="dataAllBorder02 cage_cl" style="background-color: #5bc0de">
                        <div style="align-content: center;height: 90%" id="main_charts2" class="weherChart">

                        </div>
                    </div>
                </div>
                <div class="dataAllBorder01 cage_cl check_decrease" style="overflow:scroll; margin-top: 1.5% !important; height: 20%; position: relative;">
...
<script language="JavaScript">
    function init_echarts() {
        ...
        var mychart02=echarts.init(document.getElementById('main_charts2'));
        var value1A=0,value1B=0,value1C=0,value1D=0; //pqi综合指标 各占比
        var value2A=0,value2B=0,value2C=0,value2D=0; //rqi平整度指标 各占比
        var value3A=0,value3B=0,value3C=0,value3D=0; //pci破损状况各占比
        var time_data=[]; //时间数组
        var option;
        var option2;
        var s1=[];
        ...
        $.ajax({
            ...
            success:function (data) {
                if(data!=null&&data.data.length>0){
                    $.each(data.data, function (i, list) {
                        s1.push(list.zonghe_PQI);
                        s2.push(list.pinzheng_RQI);
                        s3.push(list.poshun_PCI);
                        time_data.push(list.data);
                        if (list.pqi_dj=="A"){
                            value1A++;
                        }
                        else if(list.pqi_dj=="B"){
                            value1B++;
                        }
                        else if(list.pqi_dj=="C"){
                            value1C++;
                        }
                        else if(list.pqi_dj=="D"){
                            value1D++;
                        }
                        //
                        if (list.rqi_dj=="A"){
                        ...
                        else if(list.rqi_dj=="B"){
                            value2B++;
                        }
                        else if(list.rqi_dj=="C"){
                            value2C++;
                        }
                        else if(list.rqi_dj=="D"){
                            value2D++;
                        }
                        //
                        if (list.pci_dj=="A"){
                        ...
                        else if(list.pci_dj=="B"){
                            value3B++;
                        }
                        else if(list.pci_dj=="C"){
                            value3C++;
                        }
                        else if(list.pci_dj=="D"){
                            value3D++;
                        }
                    });
                        ...
                        series: [
                            {
                                ...
                                barGap: 0,
                                label: labelOption,
                                data: [value1A, value1B, value1C, value1D]
                            },
                            {
                                name: 'RQI',
                                type: 'bar',
                                label: labelOption,
                                data: [value2A, value2B, value2C, value2D]
                            },
                            {
                                name: 'PCI',
                                type: 'bar',
                                label: labelOption,
                                data: [value3A, value3B, value3C, value3D]
                            }
                        ]
                    };
...
//        $("#map_title_innerbox").show();
    ...
    function bootstrap_racte(id) {
        var opt={
            method: "get",
            url: "com01/response3?roid="+id+"",
            dataType: "json",
        };
        $('#table').bootstrapTable('refresh',opt);
    }
        ...
        success: function (data) {
            ...
            $.each(data.res, function (i, list) {
                ...
                marker.on('click',function (e) {
                    ...
                    // 点击事件
                    var roid_number=e.target.orderno;
                    id=roid_number;
                    acaqure(roid_number); //搜索日检
                    bootstrap_racte(roid_number); //搜索年检
